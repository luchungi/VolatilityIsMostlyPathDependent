...
def predict(β0, β1, β2, λ11, λ12, θ1, λ21, λ22, θ2, df, n, biz_days=False):
    '''
    ...
    # NOTE: the PDV paper uses business days instead of calendar days but performance is better with calendar days
    if biz_days:
        t = torch.ones(len(df) - n + 1, n, dtype=torch.float64, requires_grad=False)
    else:
...
def batch_predict(β0, β1, β2, λ11, λ12, θ1, λ21, λ22, θ2, features, datetimeindex, n, return_numpy=False, biz_days=False):
    # create sliding window of size n for r1 and r2 with shape (len(df) - n + 1, n)
    ...
    # NOTE: the PDV paper uses business days instead of calendar days but performance is better with calendar days
    if biz_days:
        t = torch.ones(len(datetimeindex) - n + 1, n, dtype=torch.float64, requires_grad=False)
    else:
...
def residual(x, df, n, return_y_hat=False, biz_days=False):
    '''
    ...
    y_hat = predict(β0, β1, β2, λ11, λ12, θ1, λ21, λ22, θ2, df, n, biz_days)
    if return_y_hat:
...
def evaluate(params, data, window, biz_days=False):
    '''
    ...
    window: int
        sliding window size to calculate R1 and R2
    '''
    residuals = residual(params, data, window, biz_days=biz_days)
...
def plot(params, data, window, valid_start_date=None, test_start_date=None, biz_days=False):
    '''
    ...
    '''
    preds = predict(*params, data, window, biz_days)
